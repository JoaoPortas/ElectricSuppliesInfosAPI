...
from scrapers.obo_bettermann_scraper import OBOBettermannScraper
from managers.SeleniumManager import SeleniumManager

obo_bettermann_bp = Blueprint("obo_bettermann", __name__)


@obo_bettermann_bp.route("/<product_id>")
def obo_bettermann(product_id):
    """
    Route to scap information about the products in the caiado Website.

    Go to /obo-bettermann/<id> to get the information about a product, where the <id> is the ID of the internal
    ...
    efapel_scraper = OBOBettermannScraper(selenium_manager.get_driver())

    print(f"Getting OBO Bettermann product info for ID {product_id} ...")
    result = efapel_scraper.scrape_product_info(product_id, reset_data_source)

    if result is not None:
        print(f"OBO Bettermann product with ID {product_id} found")
        return result.to_json()
    else:
        print(f"OBO Bettermann product with ID {product_id} not found!")
